...
func (r *rest) CreateUser(ctx *gin.Context) {
	...
	r.httpRespSuccess(ctx, http.StatusCreated, "successfully created new user", user)
}
...
func (r *rest) GetMe(ctx *gin.Context) {
	userId := ctx.MustGet("userLoggedin").(uint)
	user, err := r.uc.User.Me(int(userId))
	if err != nil {
		r.httpRespError(ctx, http.StatusInternalServerError, err)
		return
	}

	r.httpRespSuccess(ctx, http.StatusOK, "successfully get me", user)
}
